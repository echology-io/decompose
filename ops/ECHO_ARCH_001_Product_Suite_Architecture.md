...
Vanta implements a 6-stage pipeline that transforms raw documents into structured, enriched, indexed data.
...
2. **Pre-Filter** (optional) -- Decompose library filters text to high-value units; provides authority/risk profiles and standards as deterministic confidence signals.
3. **Classify** -- Deterministic-first: filename + regex + Decompose signals scored; >0.8 confidence skips AI. Falls through to Ollama with regex fallback. Entity validators reject hallucinated values.
4. **Enrich** -- Apply 5 enrichment plugins with circuit breaker protection.
5. **Decompose** -- Break documents into structural components for downstream analysis.
6. **Index** -- Embed and store in Qdrant vector collections for semantic retrieval.
...
| `vanta_classify_v2.py` | 1,359 | Deterministic-first classification (Ollama fallback)           |
