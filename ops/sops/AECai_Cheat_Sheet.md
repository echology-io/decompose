...
**Kyle Vines** — Solo technical founder/CEO. 13+ years geotechnical/civil engineering. Previously acquired a geotech firm (Vines Systems) — went through bankruptcy — turned career-ending event into product credibility. Last role: LaBella Associates (ENR Top 200). Charleston, SC, three kids. 19,000+ lines of production code, solo. Mother is CTO and cybersecurity board member — security in DNA, not bolted on.

**Core thesis**: "I'm not theorizing about the problem, I lived it and lost everything to it."
...
## The Pipeline (5 stages, every document)
...
| **Classify** | `vanta_classify_v2.py` -> `UniversalClassifier.classify_document(text, filename)` | AI-first (Ollama llama3) + regex fallback. Returns document_type, content_domains, entities, risk_indicators, suggested_plugins |
...
|------|-----------|-------------|
| `vanta_core.py` | `TextExtractor`, `OllamaClient` | Text extraction (16+ formats), chunking, standards/entity detection, envelope creation |
| `vanta_classify_v2.py` | `UniversalClassifier` | AI-first classification: doc type, domains, entities, risk, authority levels (62 AEC keywords, 60+ doc type signatures) |
