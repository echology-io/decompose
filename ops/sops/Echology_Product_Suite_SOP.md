...
`UniversalClassifier.classify_document(text, filename)` sends document text to Ollama (llama3 model) asking "what is this?" and produces a comprehensive classification envelope including `document_type`, `content_domains`, `entities`, `risk_indicators`, and `suggested_plugins`.

- **AI-first:** Uses 3-channel parity voting for classification confidence.
- **Regex fallback:** When Ollama is unavailable, falls back to regex-based authority analysis plus filename hints (60+ patterns). Every document receives a complete classification regardless of AI availability. The `classification_method` field indicates `ai_primary` or `regex_fallback`.
